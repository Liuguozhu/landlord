 ...
 * @author LGZ
 */
public class LandlordUtil4 extends LandlordBaseUtil {
    private static Logger logger = LoggerFactory.getLogger(LandlordUtil4.class);

    /**
     * Don't let anyone instantiate this class.
     */
    private LandlordUtil4() {
     ...
     *
     * @param playerId  玩家id
     * @param cardsList List<Integer> 本次出的牌
     * @return battle -AbstractBattleDouDiZhu
     */
    public static boolean checkPlayCardsType(int playerId, List<Integer> cardsList, AbstractBattleDouDiZhu battle) {
        ...
        if (lastPlayCards.size() == 0) { // 当前没有玩家出牌，当前玩家是这一圈的首次出牌
            ...
            int isStraight = checkStraight(cardsList, set, battle.straightType);
            if (isStraight > 0) {
        ...
        if (cardsList.size() == lastPlayCards.size()) {
            ...
            // 验证顺子
            if (checkStraight(cardsList, recSet, battle.straightType) > 0
                    && checkStraight(lastPlayCards, lastSet, battle.straightType) > 0) {
     ...
     * @return boolean
     */
    private static boolean compareThreeWithOne(TreeMap<Integer, Integer> recMap,
                                               TreeMap<Integer, Integer> lastMap) {
     ...
     * @return boolean
     */
    private static boolean compareThreeWithTwo(TreeMap<Integer, Integer> recMap,
                                               TreeMap<Integer, Integer> lastMap) {
        return compareTreeMap(recMap, lastMap, 3);
    }
     ...
     * @return boolean
     */
    private static boolean compareAirPlane(TreeMap<Integer, Integer> recMap,
                                           TreeMap<Integer, Integer> lastMap, List<Integer> recCards) {
        int needSerialNum = 0;
     ...
     * @return boolean
     */
    private static boolean compareFourWithTwo(TreeMap<Integer, Integer> recMap,
                                              TreeMap<Integer, Integer> lastMap) {
    ...
    private static int checkBomb(List<Integer> cardsList) {
        ...
        if (cardsList.size() == 2) {// 两张红三
            int cardValue = getCardValue(cardsList.get(0));
            if (isSameElement(cardsList) && cardValue == 3) {
                return Consts.TYPE_BOMB;
            }
        }
        return -1;
    ...
    /**
     * 验证顺子
     * XXX 四人斗地主的顺子是可以选择由几张起算顺子的，三人的顺子必须是5张起
     *
     * @param cardsList List<Integer>
     * @return 顺子返回true
     */
    private static int checkStraight(List<Integer> cardsList, Set<Integer> set, int straightType) {
        if (cardsList.size() < straightType) {
            return -1;
        }
    ...
    /**
     * 验证连对
     * XXX 四人斗地主的连对由2组对子起，三人斗地主的是3组起
     *
     * @param cardsList  List<Integer>
     * @param set        Set<Integer>
     * @param elementMap TreeMap<Integer, Integer>
     * @return int
     */
